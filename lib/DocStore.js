...
DocStore.prototype.reset = function(){
  this.db.serialize(function(){
    ...
    this.db.get('CREATE TABLE lineage( id INTEGER, pid INTEGER );');
    this.db.get('CREATE TABLE tokens( id INTEGER, token STRING );');
  }.bind(this));
};
...
DocStore.prototype.setTokens = function( id, tokens, cb ){
  ...
  this.db.run(
    'INSERT INTO tokens ( id, token ) VALUES ' + tokens.map( token => {
      return '(' + id + ',"' + token + '")';
    }).join(',') + ';',
    cb
  );
