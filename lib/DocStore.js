...
DocStore.prototype.reset = function(){
  this.db.serialize(function(){
    ...
    this.db.run('CREATE VIRTUAL TABLE fulltext USING fts5(token, tokenize="unicode61 tokenchars \' \'", prefix="1 2 3 4 5");');

    // this.db.run('CREATE INDEX IF NOT EXISTS lineage_id_idx ON lineage(id);');
    // this.db.run('CREATE INDEX IF NOT EXISTS lineage_pid_idx ON lineage(pid);');
    // this.db.run('CREATE INDEX IF NOT EXISTS lineage_cover_idx ON lineage(id, pid);');
    // this.db.run('CREATE INDEX IF NOT EXISTS tokens_id_idx ON tokens(id);');
    // this.db.run('CREATE INDEX IF NOT EXISTS tokens_lang_idx ON tokens(lang);');
    // this.db.run('CREATE INDEX IF NOT EXISTS tokens_cover_idx ON tokens(id, lang);');
    // this.db.run('CREATE INDEX IF NOT EXISTS tokens_cover_idx ON tokens(id, lang, token);');
...
DocStore.prototype.preCommit = function(){
  this.db.serialize(function(){
    ...
    this.db.run('INSERT INTO fulltext(rowid, token) SELECT rowid, token FROM tokens;');
